...
{
	if(!headers_sent())
		header('Location: ' . Uri::ObtenerRutaARaiz() . '/Panel');
	exit;
}
...
<title>Gestión de permisos de grupo - Club Orión de Astronomía</title>
<!-- InstanceEndEditable -->
...
?></li>
        ...
        <h1>Permisos de grupo: <?php echo $grupo->Nombre; ?></h1>
        <p>Estos permisos se aplicarán a cualquier miembro que esté en este grupo.</p>
<?php
...
?>
          <tr>
            <td><?php echo "<strong>$permiso[0]</strong>"; if($permiso[1] != null) echo "<br/>$permiso[1]"; ?></td>
            <td><div id="bp<?php echo $permiso[0]; ?>"></div><script type="text/javascript">$("#bp<?php echo $permiso[0]?>").BotonAjax({Url: '../../../Administracion/PermisoGrupoHdlr.json', Datos: {p: '<?php echo $permiso[0]; ?>', g: <?php echo $grupo->Id; ?>}, Estado: '<?php echo (Permiso::OtorgadoSoloGrupo($grupo, $permiso[0])) ? 'Activado' : 'Desactivado'; ?>'});</script></td>
          </tr>
<?php
	}
?>
...
<?php
	...
	echo MensajeHtml::JqueryUiError('El sistema no tiene registrado permisos para aplicar');
?>
