...
$usuarios = UsuarioFacebook::ObtenerUsuariosRegistrados();
?>
...
<title>Lista de usuarios - Club Orión de Astronomía</title>
<!-- InstanceEndEditable -->
...
$(document).ready(function(){
	...
	$(".BotonAjax").click(function() {
		if(!$(this).data('BotonAjax').t){
			var c = false;
			if($(this).data('BotonAjax').Estado == 'Desactivado') {
				c = confirm("Bajo un gran poder hay una gran responsabilidad. ¿Estás realmente seguro de otorgarle TODOS LOS PERMISOS a este usuario?");
			} else {
				c = true;
			}		
			if(c) {
			...
			var cc = false;
			if($(this).data('BotonAjax').Estado == 'Activado') {
				cc = confirm("¡Advertencia! Si marca a este usuario como no integrante del club, se borrarán sus permisos personalizados así como cualquier vinculación a grupos que tenga. Se deshabilitará el ñoñómetro pero no se perderán los puntos de experiencia. ¿Está seguro de realizar esta acción?");
			} else {
				cc = true;
			}
			if(cc) {
...
?></li>
        ...
        <p>Esta lista muestra la información de cada usuario que ha ingresado al sistema y su vinculación con su cuenta de Facebook.</p>
        <table class="TablaAccion">
          <tr>
            <th scope="col" style="width: 30%">Usuario</th>
            <th scope="col" style="width: 10%">Vínculo</th>
            <th scope="col" style="width: 10%">Integrante</th>
            <th scope="col" style="width: 10%">Súper</th>
...
foreach($usuarios as $usuario)
{
	try
	{
		$data = $usuario->DatosFacebook;
?>
          <tr>
            <td><?php $itemXIL = $usuario->GenerarItemXInLine(); echo $itemXIL->ATexto(true, false); if($usuario->EsMiembroClubOrion) { $x = $usuario->Experiencia->GenerarBarraProgresoX(); echo $x->ATexto(true, false); } ?></td>
            <td style="text-align:center"><?php echo $usuario->ObtenerEstadoVFb(); ?></td>
            <td><div id="bv<?php echo $usuario->IdFacebook; ?>"></div><script>$("#bv<?php echo $usuario->IdFacebook; ?>").BotonAjax({t: 'bv', Url: '../../../Administracion/IntegracionHdlr.json', Datos: {u: '<?php echo $usuario->IdFacebook; ?>'}, Estado: '<?php echo ($usuario->EsMiembroClubOrion) ? 'Activado' : 'Desactivado'; ?>'});</script></td>
            <td><div id="bsu<?php echo $usuario->IdFacebook; ?>"></div><script>$("#bsu<?php echo $usuario->IdFacebook; ?>").BotonAjax({Url: '../../../Administracion/SuperUsuarioHdlr.json', Datos: {u: '<?php echo $usuario->IdFacebook?>'}, Estado: <?php echo ($usuario->EsSuperUsuario) ? '\'Activado\'' : '\'Desactivado\''; ?>});</script></td>
            <td style="text-align:center"><?php echo $usuario->ObtenerPuntosDisponibles(); ?></td>
            <td style="text-align:center"><a href="Permisos.php?u=<?php echo $usuario->IdFacebook; ?>">Editar permisos</a></td>
          </tr>
<?php
	}
	catch(Exception $e)
