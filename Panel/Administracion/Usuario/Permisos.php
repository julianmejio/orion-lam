...
<title>Gestión de permisos - Club Orión de Astronomía</title>
<!-- InstanceEndEditable -->
...
{
?>
		alert("No se pueden gestionar permisos individuales de un súper usuario. Primero debe retirar lo súper del usuario para poder gestionar sus permisos individualmente");
<?php
}
else
{
...
?></li>
        ...
        <h1>Gestionar permisos: <?php echo $usuario->DatosFacebook['name']; ?></h1>
<?php
...
?>
        <table class="TablaAccion">
          <tr>
            <th scope="col" style="width: 90%">Permiso</th>
            <th scope="col" style="width: 10%">Otorgado</th>
          </tr>
<?php
...
?>
          <tr>
            <td><strong><?php echo $permiso[0]; ?></strong><?php if($permiso[1] != null) echo "<br />$permiso[1]"; ?></td>
            <td><div id="bp<?php echo $permiso[0]; ?>"></div><script>$("#bp<?php echo $permiso[0]; ?>").BotonAjax({Url: '../../../Administracion/PermisoHdlr.json', Datos: {p: '<?php echo $permiso[0]; ?>', u: '<?php echo $usuario->IdFacebook; ?>'}, Estado: '<?php echo ($usuario->TienePermiso($permiso[0])) ? 'Activado' : 'Desactivado'; ?>'});</script></td>
          </tr>
<?php
	}
?>
...
<?php
	...
	echo MensajeHtml::JqueryUiError('El sistema no tiene permisos');
?>
