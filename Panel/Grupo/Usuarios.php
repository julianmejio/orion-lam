...
<title>Usuarios de grupo - Club Orión de Astronomía</title>
<!-- InstanceEndEditable -->
...
if(isset($_REQUEST['g']))
{
	...
	{
		$membresia = false;
		if($grupo->EsAdministrador($usuarioActivo) || $usuarioActivo->TienePermiso('AdministrarGrupos'))
			$membresia = 'Administrador';
		else if($grupo->EsMiembro($usuarioActivo) || $usuarioActivo->TienePermiso('VerGrupos'))
			$membresia = 'Miembro';
		switch($membresia)
		{
			case 'Administrador':
				$usuarios = UsuarioFacebook::ObtenerMiembros();
?>
...
?>
        <table class="TablaAccion">
          <tr>
            ...
            <th scope="col" style="width: 20%">Pertenece</th>
            <th scope="col" style="width: 20%">Administrador</th>
          </tr>
<?php
...
?>
          <tr>
            <td><?php $itemX = $usuario->GenerarItemXInLine(); echo $itemX->ATexto(true, false); ?></td>
            <td><div id="bgp<?php echo $usuario->IdFacebook; ?>"></div><script type="text/javascript">$("#bgp<?php echo $usuario->IdFacebook; ?>").BotonAjax({Url: '../../Grupo/BotonHdlr.json', Datos: {Usuario: "<?php echo $usuario->IdFacebook; ?>", Grupo: <?php echo $grupo->Id; ?>, Accion: 'Pertenencia'}, Estado: '<?php echo ($grupo->EsMiembro($usuario)) ? 'Activado' : 'Desactivado'; ?>'});</script></td>
            <td><div id="bga<?php echo $usuario->IdFacebook; ?>"></div><script type="text/javascript">$("#bga<?php echo $usuario->IdFacebook; ?>").BotonAjax({Url: '../../Grupo/BotonHdlr.json', Datos: {Usuario: "<?php echo $usuario->IdFacebook; ?>", Grupo: <?php echo $grupo->Id; ?>, Accion: 'Administracion'}, Estado: '<?php echo ($grupo->EsAdministrador($usuario)) ? 'Activado' : 'Desactivado'; ?>'});</script></td>
          </tr>
<?php
					}
?>
...
<?php
					...
					echo MensajeHtml::JqueryUiError('No hay usuarios registrados.');
				break;
			case 'Miembro':
?>
        <h1>Grupo: <?php echo $grupo->Nombre; ?></h1>
...
?>
          <tr>
            <td><?php $itemX = $usuario->GenerarItemXInLine(); echo $itemX->ATexto(true, false); ?></td>
            <td><span title="No disponible">Hoja de vida</span></td>
          </tr>
<?php
				}
?>
...
<?php
				break;
			default:
				echo MensajeHtml::JqueryUiError('¡No eres miembro de este grupo!');
				break;
		}
	}
	else
		echo MensajeHtml::JqueryUiError('El grupo no es válido');
}
else
	echo MensajeHtml::JqueryUiError('No se especificó ningún grupo.');
?>
